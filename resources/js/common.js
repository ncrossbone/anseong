var common = (function(){
	...
	return{
		...
		dynamicTreePanel3Controll: function(node){
			asSgis.getApplication().fireEvent('dynamicLayer3OnOff', node);
		},
		...
		searchTreePanel1Control2: function(node){
			asSgis.getApplication().fireEvent('searchLayerOnOff_2', node);
		},
		...
		searchTreePanel3Controll: function(node){
			asSgis.getApplication().fireEvent('searchTryLayerOnOff', node);
		},
		...
		pollutionPop: function(grid,getCoId, noCoId){//(해당 그리드 , coId를 갖고 있는지)
			...
			var pollutionDetailLayer = [_API.AREA_BASE,_API.JIMOK_CHG,_API.OWN_CHG];
		...
		mapClick: function(evt,type){
			...
			var selectLayer_2 = coreMap.searchLayerAdmin.layer_2.visibleLayers;
			
			var sumLayers = selectLayer.concat(selectLayer_2);
			...
			if(type == "search"){
				asSgis.getApplication().fireEvent('bufferPoint', evt);
			}else{
			...
			if(sumLayers.length > 0){
				...
				for(var i = 0; i < sumLayers.length ; i++){
						...
						var queryTask = new esri.tasks.QueryTask(_API.searchLayer + "/" + sumLayers[i]);
						var query = new esri.tasks.Query();
						if(type == "search"){
							query.where = "PNU = '"+evt+"'";					
						}else{
					...
					try{
						for(var i=0; i<arguments[0].length; i++){
							...
							if(resultValue[0] != undefined){
								resultValue[0].attributes.layerId = sumLayers[i];
							}
							results = results.concat(resultValue);
		...
		searchResult: function(layerId, PNU , getcoIdLayer , noCoId , type){
			...
			if(layerId == _API.AREA_STATE){ //예외 legend에 포함되어 있지 않은 table 정보
				title = "오염시설이력";
			}
		...
		excelDownLoad: function(){
			...
			var tabCtl = Ext.getCmp("searchResultTabPanel");
			console.info(tabCtl);
			...
			var tabpanels = Ext.getCmp("searchResultTabPanel");
			...
			this.excelDown(ClTitle[0], headName, header, datas);
		...
		excelDown: function (fileNm, headName, header, datas) {
			...
			var obj = {'header':JSON.stringify(header), 'headerNm':JSON.stringify(headName), 'datas':JSON.stringify(datas)};
			$.post('./resources/data/excelDown.jsp', obj, function(data){
